public class LatihanPerkalianMatrix {

    /**
     * @param args the command line arguments
     */
    public static int[][] perkalianMatriks(int [][] A, int [][] B) throws Exception {
        if (A.length==0 || B.length==0 || A[0].length==0 || B[0].length==0) {
            throw new Exception ("A dan B tidak boleh kosong");
        } else if (A[0].length != B.length) {
            throw new Exception ("A dan B tidak bisa di kalikan karena dimensi tidak cocok");
        }
        
        int [][] C = new int [A.length] [B[0].length];
        for (int i = 0; i < A.length; i++) {
            for (int j = 0; j < B[0].length; j++) {
                C[i][j]=0;
                for (int k = 0; k < A[i].length; k++) {
                    C[i][j] += A[i][k]* B[k][j];
                }
            }
        }
        
        return C;
    }
    
    public static void cetakArray2D(int A[][]) {
        for (int i = 0; i < A.length; i++) {
            for (int j = 0; j < A[i].length; j++) {
                System.out.print(A[i][j] + " ");
            }
            System.out.println("");
        }
    }
    
    public static void main(String[] args) {
       int A[][] = {{4,3,1}, {2,6,5}};
       int B[][] = {{2,5,4,6}, {8,2,7,0}, {7,1,3,9}};
       
       try  {
           int C[][] = perkalianMatriks(A, B);
           cetakArray2D(C);
       }
       catch (Exception ex) {
           System.err.println(ex.toString());
       }
    }
}