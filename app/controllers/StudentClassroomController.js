...
export const update = async (req, res) => {
  ...
  const studentClassroom = await StudentClassroom.findOne({
    where: {
  ...
  if (!studentClassroom) {
    return res.status(404).json({
      success: false,
      message:
        "Update student in classroom failed, User or Classroom not found",
    });
  }
  ...
  if (!class_code || !uid) {
    return res.status(400).json({
      success: false,
      message: "Update student in classroom failed, Field cannot empty",
    });
  }
  ...
  if (noUser) {
    return res.status(400).json({
      success: false,
      message: "Update student in classroom failed, User already in classroom",
    });
  }
  ...
  if (!student || !classroom) {
    return res.status(404).json({
      success: false,
      message:
        "Update student in classroom failed, UID or Class code not found",
    });
  }
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Update student in classroom successfully",
    });
  } catch (error) {
    console.log(error.message);
    res.status(500).json({
      success: false,
      message: "Update student in classroom failed",
    });
  }
};
